...
test_binary "unzip" "-v"
test_binary "bun" "--version"

# Test that bun can run TypeScript type-checking
echo -n "Testing bun x tsc... "
if output=$(docker run --rm project-container:test bun x tsc --version 2>&1); then
  echo "OK"
else
  exit_code=$?
  echo "FAIL (exit code: $exit_code)"
...
# Test that bun can run eslint
echo -n "Testing bun x eslint... "
if output=$(docker run --rm project-container:test bun x eslint --version 2>&1); then
  echo "OK"
else
  exit_code=$?
  echo "FAIL (exit code: $exit_code)"
